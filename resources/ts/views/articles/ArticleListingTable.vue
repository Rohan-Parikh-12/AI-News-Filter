...
watch([searchQuery, filterCat, showSaved], () => { page.value = 1; fetchData() })
    ...
    <VCard>
      ...
      >
        ...
        <template #item.title="{ item }">
          <div class="d-flex align-center gap-3 py-2" style="max-inline-size: 320px;">
            <VAvatar
              rounded
              size="40"
            ...
            <div>
              <a
                :href="item.url"
                target="_blank"
                class="text-body-2 font-weight-medium text-high-emphasis d-block text-wrap"
                style="max-inline-size: 260px; line-height: 1.4;"
              >
                {{ item.title }}
              </a>
        ...
        <template #item.summary="{ item }">
          <div style="max-inline-size: 280px; white-space: normal;">
            <span v-if="item.summary" class="text-body-2">
              {{ item.summary.slice(0, 90) }}{{ item.summary.length > 90 ? '...' : '' }}
            </span>
        ...
        <template #item.published_at="{ item }">
          <span class="text-body-2">{{ item.published_at }}</span>
        </template>
        ...
        <template #item.actions="{ item }">
          <IconBtn
            size="small"
            :color="item.is_saved ? 'primary' : 'default'"
